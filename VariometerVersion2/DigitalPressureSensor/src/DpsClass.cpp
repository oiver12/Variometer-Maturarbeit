...
int16_t DpsClass::measurePressureOnce(float &result, uint8_t oversamplingRate)
{
	...
	//wait until measurement is finished
	//Serial.println((calcBusyTime(0U, m_prsOsr) / DPS__BUSYTIME_SCALING) + DPS310__BUSYTIME_FAILSAFE);
...
int16_t DpsClass::startPressureWithoutDelay(float &delay)
{
	//start the measurement
	int16_t ret = startMeasurePressureOnce(1);
	if (ret != DPS__SUCCEEDED)
	{
		return ret;
	}
	delay = (calcBusyTime(0U, m_prsOsr) / DPS__BUSYTIME_SCALING) + DPS310__BUSYTIME_FAILSAFE;
	return ret;
}

int16_t DpsClass::getResultWhenStarted(float &result)
{
	int16_t ret = getSingleResult(result);
	if (ret != DPS__SUCCEEDED)
	{
		standby();
	}
...
{
	return writeByteBitfield(1U, config_registers[FIFO_ENDPS]);
}

int16_t DpsClass::disableFIFO()
{
	int16_t ret = flushFIFO();
	ret = writeByteBitfield(0U, config_registers[FIFO_ENDPS]);
	return ret;
}
...
int16_t DpsClass::readBlock(RegBlock_t regBlock, uint8_t *buffer)
{
	...
	int16_t ret = m_i2cbus->requestFrom(m_slaveAddress, regBlock.length, true);
